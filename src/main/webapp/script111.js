...
// Change url api here
//var root = 'https://my-json-server.typicode.com/teppinguyen33/dummyapi/data';
...
$(document).ready(function () {
  ...
  $(document).on('click', '.js-edit', function (e) {
    ...
    $.each(workHis, function (key, value) {
      if (key === 'name') {
        $padd.find('select[name=' + key + ']').val(value);
      }
      if (key === 'money') {
  ...
  // Enabled/disabled button addWork
  $('.js-workHisName, js-workHisMoney').on('change', function () {
    checkWorkHis()
  })

  // Collect data and call addWork or updWork depend on having workHisId or not
